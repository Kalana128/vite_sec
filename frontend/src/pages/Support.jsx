...
export default function Support() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!sanitizedName || !sanitizedEmail || !sanitizedMessage) {
      setWarning('All fields except image are required.'); // Set warning
      return;
    }

    if (!validateEmail(sanitizedEmail)) {
      setWarning('Please enter a valid email address.'); // Set warning
      return;
    }

    // Check for malicious input
    if (sanitizedName.includes('<script>') || sanitizedMessage.includes('<script>')) {
      setWarning('Malicious input detected. Please refrain from using harmful scripts.'); // Set warning
      return;
    }
  ...
  const handleImageUpload = (e) => {
    ...
    const allowedTypes = ['image/jpeg', 'image/jpg', 'image/png'];
    if (file && !allowedTypes.includes(file.type)) {
      setWarning('Only JPEG, JPG, and PNG files are allowed.'); // Set warning
      return;
    }
    ...
    const maxSize = 2 * 1024 * 1024; // 2MB
    if (file && file.size > maxSize) {
      setWarning('File size exceeds 2MB. Please choose a smaller file.'); // Set warning
      return;
    }

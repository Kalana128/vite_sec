...
export default function FinalAssessment() {
  ...
  const handleSubmit = () => {
    const newScore = quizQuestions.reduce((acc, question, index) => {
      if (question.type === 'multiple-choice') {
        return selectedOptions[index] === question.answer ? acc + 1 : acc;
      } else if (question.type === 'true-false') {
        return selectedOptions[index].toLowerCase() === question.answer.toString() ? acc + 1 : acc;
      }
      return acc;
    }, 0);
    setScore(newScore);
  ...
  const handleRetry = () => {
    ...
    setSelectedOptions(Array(newQuestions.length).fill(''));
...
//             {quizQuestions.map((question, index) => (
//     <div key={index} className='border rounded-lg p-4 bg-slate-100 mb-4'>
//       <h3 className='font-semibold text-xl'>{index + 1}. {question.question}</h3>
//       <div className='flex flex-col mt-2'>
//         {question.type === 'multiple-choice' ? (
//           question.options.map((option, optionIndex) => (
//             <label key={optionIndex} className='flex items-center mb-2'>
